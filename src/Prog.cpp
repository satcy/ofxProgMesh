...
Prog::Prog(Mesh* mesh, EdgeCost ec)
{
    assert(mesh);
...
void Prog::setup(Mesh* mesh, EdgeCost ec)
{
    assert(mesh);
    assert(ec >= 0 && ec < MAX_EDGECOST);
...
void dumpset(vertexPtrSet& ms)
{
    ...
    for (iter = ms.begin(); iter != ms.end(); ++iter)
    {
        ...
        std::cout << v._index;
        const Vertex& vtx = v._meshptr->getProgVertex(v._index);
...
void Prog::setProgress(float _percent){
    if ( _percent == percent ) return;
    float pre_percent = percent;
    percent = _percent;
    float target_rate = ofClamp(percent, 0, 100) / 100.0;
    int mode = 0;
    if ( pre_percent > _percent ) mode = 0;
    else mode = 1;
    
    bool ret = true;
    //for (int i = 0; ret && i < size; ++i) {
    float t_num = numTris() * target_rate;
    while ( ret ) {
        if ( mode == 0 ) ret = collapseEdge();// --
        else ret = splitVertex();
        if ( ret ) {
            if ( mode == 0 ) ret = numVisTris() > t_num;
            else ret = numVisTris() < t_num;
        }
    }
}
    
void Prog::readToMesh(ofMesh & mesh){
    ...
    for ( int i =0, cnt=0; i<num; i++ ) {
        ...
        if ( t.isActive() ) {
            ...
            mesh.addVertex(ofVec3f(v1.getXYZ().x, v1.getXYZ().y, v1.getXYZ().z));
            v1 = t.getVert2vertex();
            mesh.addVertex(ofVec3f(v1.getXYZ().x, v1.getXYZ().y, v1.getXYZ().z));
            v1 = t.getVert3vertex();
            mesh.addVertex(ofVec3f(v1.getXYZ().x, v1.getXYZ().y, v1.getXYZ().z));
            
            ofVec3f norm = t.getNormalVec3();
            
            mesh.addNormal(ofVec3f(norm.x, norm.y, norm.z));
            mesh.addNormal(ofVec3f(norm.x, norm.y, norm.z));
            mesh.addNormal(ofVec3f(norm.x, norm.y, norm.z));
...
#ifndef NDEBUG
void Prog::assertEveryVertActive(int nVerts, int nTri, Mesh &mesh)
{
    int i;
    for (i = 0; i < nVerts; ++i)
    {
        assert(mesh.getProgVertex(i).isActive());
    }
    for (i = 0; i < nTri; ++i)
    {
...
// collapse costs.
void Prog::calcQuadricMatrices(EdgeCost &cost, Mesh &mesh)
{
    if (QUADRICTRI == cost)
    {
...
// are collapsed first.
void Prog::calcEdgeCollapseCosts(vertexPtrSet &vertSet, vector<vertexPtrSet::iterator> &vertSetVec,
                                  int nVerts, Mesh &mesh, EdgeCost &cost)
{
    int i;
    for (i = 0; i < nVerts; ++i)
    {
        Vertex& currVert = mesh.getProgVertex(i);
        switch (cost)
        {
            case SHORTEST:
...
// separate vertex.
void Prog::insureEdgeCollapseValid(EdgeCollapse &ec, Vertex &vc, Mesh &mesh,
                                    const EdgeCost &cost, bool &bBadVertex)
{
    ...
    for (;;) // at most this will loop twice -- the "to vertex" could have been removed, so it may need to be recalculated.
    {
        ...
        if (nLoopCount > 2 || !mesh.getProgVertex(ec._vfrom).isActive())
        {
            bBadVertex = true;
            break;
        }
        ...
        if (!mesh.getProgVertex(ec._vto).isActive())
        {
            switch (cost)
            {
                case SHORTEST:
...
// original algorithm.
void Prog::setToVertexQuadric(Vertex &to, Vertex &from, const EdgeCost &cost)
{
...
void Prog::updateTriangles(EdgeCollapse &ec, Vertex &vc, set<int> &affectedVerts, Mesh &mesh)
{
    ...
    for (pos = triNeighbors.begin(); pos != triNeighbors.end(); ++pos)
    {
        ...
        else
        {
            ...
            mesh.getProgVertex(ec._vto).addTriNeighbor(triIndex);
        ...
        if (bRemoveTri)
        {
            mesh.getProgVertex(vert1).removeTriNeighbor(triIndex);
            mesh.getProgVertex(vert2).removeTriNeighbor(triIndex);
            mesh.getProgVertex(vert3).removeTriNeighbor(triIndex);
        }
    }
}
...
void Prog::updateAffectedVertNeighbors(Vertex &vert, const EdgeCollapse &ec,
...
void Prog::resetAffectedVertCosts(const EdgeCost &cost, Mesh &mesh, Vertex &vert)
{
    switch (cost)
    {
        case SHORTEST:
...
void Prog::removeVertIfNecessary(Vertex &vert, vertexPtrSet &vertSet,
                                  ...
                                  set<int> &affectedQuadricVerts)
{
    ...
    if (bActiveVert) { // if vert is active
        ...
        mesh.getProgVertex(vp._index).setActive(true);
...
#endif
        mesh.getProgVertex(vert.getIndex()).setActive(false);
    }
}

// Update the vertices affected by the most recent edge collapse
void Prog::updateAffectedVerts(Mesh &mesh, vector<vertexPtrSet::iterator> &vertSetVec,
                                ...
                                set<int> &affectedQuadricVerts)
{
    ...
    for (mappos = affectedVerts.begin(); mappos != affectedVerts.end(); ++mappos)
    {
        ...
        Vertex& vert = mesh.getProgVertex(*mappos);
...
void Prog::recalcQuadricCollapseCosts(set<int> &affectedQuadricVerts,
                                       Mesh &mesh, const EdgeCost &cost)
{
    if (QUADRIC == cost || QUADRICTRI == cost)
    {
...
void Prog::buildEdgeCollapseList(Mesh &mesh, const EdgeCost &cost,
                                  ...
                                  vector<vertexPtrSet::iterator> &vertSetVec)
{
    for (;;)
    {
        ...
        Vertex vc = mesh.getProgVertex(vp._index);
        ...
        mesh.getProgVertex(ec._vfrom).setActive(false);
        ...
        Vertex& to = mesh.getProgVertex(ec._vto);
        Vertex& from = mesh.getProgVertex(ec._vfrom);
...
void Prog::createEdgeCollapseList()
{
    ...
    int nVerts = _newmesh.getNumVertices();
...
// for each vertex
void Prog::calcAllQMatrices(Mesh& mesh, bool bUseTriArea)
{
...
void Prog::applyBorderPenalties(set<Border> &borderSet, Mesh &mesh)
{
    ...
    for (pos = borderSet.begin(); pos != borderSet.end(); ++pos)
    {
        ...
        Vertex& v1 = mesh.getProgVertex(edgeInfo.vert1);
        Vertex& v2 = mesh.getProgVertex(edgeInfo.vert2);
        
        ofVec3f &vec1 = v1.getXYZ();
        ofVec3f &vec2 = v2.getXYZ();
        
        ofVec3f edge = vec1 - vec2;
        
        Triangle &tri = mesh.getTri(edgeInfo.triIndex);
        ofVec3f normal = tri.getNormalVec3();
        
        ofVec3f abc = unitcross(edge, normal);
        float &a = abc.x;
        float &b = abc.y;
        float &c = abc.z;
...
double Prog::shortEdgeCollapseCost(Mesh& m, Vertex& v)
{
    ...
    {
        Vertex& n = m.getProgVertex(*pos);
        if (!n.isActive()) continue;
        if (n == v) continue;
        
        // calc cost
        ofVec3f s = v.getXYZ() - n.getXYZ();
        float cost = s.length();
        ...
        if (cost < mincost)
        {
            ...
            assert(v.minCostEdgeVert() >= 0 && v.minCostEdgeVert() < m.getNumVertices());
        }
    }
...
// belongs.
void Prog::calcMelaxMaxValue(Mesh &mesh, set<int> &adjfaces,
    ...
    else
    {
        ...
        for (pos2 = tneighbors.begin(); pos2 != tneighbors.end(); ++pos2)
        {
            ...
            for (pos3 = adjfaces.begin(); pos3 != adjfaces.end(); ++pos3)
            {
                ...
                ofVec3f n(t.getNormalVec3());
                ofVec3f n2(t3.getNormalVec3());
...
double Prog::melaxCollapseCost(Mesh& mesh, Vertex& v)
{
    ...
    for (pos = vneighbors.begin(); pos != vneighbors.end(); ++pos)
    {
        ...
        {
            ofVec3f v1 = v.getXYZ();
            ofVec3f v2 = mesh.getProgVertex(*pos).getXYZ();
            ofVec3f v3 = v1 - v2;
...
double Prog::quadricCollapseCost(Mesh& m, Vertex& v)
{
    // get list of all active neighbors
...
// by the vertex again.
double Prog::calcQuadricError(double Qsum[4][4], Vertex& v, double triArea)
{
    double cost;
